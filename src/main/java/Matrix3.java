...
public class Matrix3 implements Serializable {
     ...
     * @return This matrix for the purpose of chaining operations. */
    public Matrix3 setToTranslation (Vector2D translation) {
        this.val[M00] = 1;
        this.val[M10] = 0;
        this.val[M20] = 0;
    ...
    public Matrix3 trn (Vector2D vector) {
        val[M02] += vector.x;
        val[M12] += vector.y;
        return this;
    }
     ...
     * @param translation The translation vector.
     * @return This matrix for the purpose of chaining. */
    public Matrix3 translate (Vector2D translation) {
        tmp[M00] = 1;
        tmp[M10] = 0;
        tmp[M20] = 0;
     ...
     * @return This matrix for the purpose of chaining. */
    public Matrix3 scale (Vector2D scale) {
        tmp[M00] = scale.x;
        tmp[M10] = 0;
        tmp[M20] = 0;
        tmp[M01] = 0;
     ...
     * @param scale The {@link Vector2D} to use to scale this matrix.
     * @return This matrix for the purpose of chaining methods together. */
    public Matrix3 scl (Vector2D scale) {
        val[M00] *= scale.x;
        val[M11] *= scale.y;
        return this;
    }
    ...
    public Vector2D multiply(Vector2D right) {
        ...
        result.x = right.x * val[0] + right.y * val[1] + 1 * val[2];
        result.x = right.x * val[3] + right.y * val[4] + 1 * val[5];
        result.x = right.x * val[6] + right.y * val[7] + 1 * val[8];
        return result;
    }
}