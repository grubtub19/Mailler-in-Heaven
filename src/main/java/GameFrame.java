...
import com.github.strikerx3.jxinput.XInputDevice; // Class for XInput 1.3. Legacy for Win7.
import com.github.strikerx3.jxinput.XInputDevice14; // Class for XInput 1.4. Includes 1.3 API.
...
public abstract class GameFrame extends JFrame implements Runnable {
	...
	protected LinkedList<Integer> pressed = new LinkedList<Integer>();
	protected LinkedList<Integer> justReleased = new LinkedList<Integer>();
	protected LinkedList<Integer> justPressed = new LinkedList<Integer>();
	
	//protected XboxController xc;
	XInputDevice device = null;
	protected XInputComponents components;
	protected XInputButtons buttons;
	protected XInputAxes axes;
	protected static double direction = 0;
	protected static double magnitude = 0;
	protected static int butA = 0;
	protected static int butB = 0;
	protected static int butX = 0;
	protected static boolean leftTrigger = false;
	protected static boolean rightTrigger = false;
	...
	public GameFrame(long period) {
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				...
				while(!justReleased.isEmpty()) {
					justReleased.removeFirst();
				}
	...
	public double calcDirection(double x, double y) {
		return Math.toDegrees(Math.atan2(y, -x)) + 270;
	}
	...
	public GraphicsDevice getGC() {
		return gd;
	}
	
	protected void pollInputs() {
		device.poll();
		if(calcMagnitude(axes.lx,axes.ly) > 0.2) {
			GameFrame.magnitude = calcMagnitude(axes.lx,axes.ly);
			GameFrame.direction = calcDirection(axes.lx,axes.ly);
		} else {
			GameFrame.magnitude = 0;
		}
		if(axes.lt > 0.1) {
    		leftTrigger = true;
    	}
	...
	private void gameRender(Graphics2D gScr) {
		...
		gScr.setColor(new Color(139, 183, 122));
		gScr.fillRect(0, 0, pWidth, pHeight);
		gScr.setColor(Color.blue);
		gScr.drawString("" + averageFPS, 50, 50);
		simpleRender(gScr);
	} // end of gameRender()
	...
	private void screenUpdate() {
		...
		try {
			...
			gScr.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
	...
	// attempt to set the display mode to the given width, y_offset, and bit depth
	{
	...
	/*
	 * Check that a displayMode with this width, y_offset, bit depth is available.
